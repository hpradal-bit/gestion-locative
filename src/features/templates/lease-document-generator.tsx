...
export function LeaseDocumentGenerator({
  ...
  }, [templateId, leaseId, checkVersion]);
  ...
  return (
    <div className="flex flex-col gap-6">
        ...
        <CardContent className="flex flex-col gap-4">
          ...
          <div className="flex flex-wrap justify-end gap-2">
            <Button variant="outline" disabled={!ready} asChild={ready}>
              {ready ? (
                <a
                  href={`/api/modeles/${templateId}/generer?leaseId=${leaseId}`}
                  target="_blank"
                  rel="noreferrer"
                >
                  <FileOutput />
      ...
      {leaseId && (
        <Card>
          <CardHeader>
            <CardTitle>Vérifier et modifier les informations</CardTitle>
            <CardDescription>
              Tous les champs qui alimentent le bail, au même endroit — propriétaire, locataire,
              bien et bail. Une modification ici met à jour la fiche correspondante et se reflète
